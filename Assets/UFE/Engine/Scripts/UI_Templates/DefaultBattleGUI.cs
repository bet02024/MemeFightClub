...
public class DefaultBattleGUI : BattleGUI{
	...
	protected override void OnGameBegin (ControlsScript cPlayer1, ControlsScript cPlayer2, StageOptions stage){
		...
		if (this.player1GUI != null && this.player1GUI.address != null){
			this.player1GUI.address.text = UFE.address1;
		}

		if (this.player2GUI != null && this.player2GUI.address != null){
			this.player2GUI.address.text = UFE.address2;
		}
	...
	protected override void OnGameEnd (ControlsScript winner, ControlsScript loser){
		...
		//winner.playerNum == this.player1.controlsScript.playerNum

		string winnerAddress = UFE.addressFull1;
		string loserAddress = UFE.addressFull2;
		if (winner.playerNum == 2){
			winnerAddress = UFE.addressFull2;
			loserAddress = UFE.addressFull1;
		}

		Debug.LogError("##Winner ");
		Debug.LogError(winner.playerNum);
		Debug.LogError(winnerAddress);

		FirebaseFirestore db = FirebaseFirestore.DefaultInstance;

		DocumentReference leadersRef = db.Collection("leaders").Document(winnerAddress);
		leadersRef.GetSnapshotAsync().ContinueWithOnMainThread(task =>
		{
			DocumentSnapshot snapshot = task.Result;
			if (snapshot.Exists) {
				long wons = snapshot.GetValue<long>("wons") + 1;
				Dictionary<string, object> updates = new Dictionary<string, object>
				{
					{ "wons", wons},
					{"address", winnerAddress}
				};
				leadersRef.SetAsync(updates, SetOptions.MergeAll);
			} else {
				Dictionary<string, object> updates = new Dictionary<string, object>
				{
					{ "wons", 1},
		...
		DocumentReference leadersRef2 = db.Collection("leaders").Document(loserAddress);
		leadersRef2.GetSnapshotAsync().ContinueWithOnMainThread(task =>
		{
			DocumentSnapshot snapshot = task.Result;
			if (snapshot.Exists) {
				long lost = snapshot.GetValue<long>("lost") + 1;
				Dictionary<string, object> updates = new Dictionary<string, object>
				{
					{ "lost", lost},
					{"address", loserAddress}
				};
				leadersRef2.SetAsync(updates, SetOptions.MergeAll);
			} else {
				Dictionary<string, object> updates = new Dictionary<string, object>
				{
					{ "wons", 0},
